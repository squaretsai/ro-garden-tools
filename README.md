...
建議優先使用 [啟動百分比經驗計算.cmd](./啟動百分比經驗計算.cmd)。這個工具不依賴 `/savechat`，只用你在遊戲畫面看到的 Base 百分比與時間計算 `%/hr`：

1. 輸入目前 Base 等級。
2. 輸入起始 Base 百分比，例如 `82.8`。
3. 輸入結束 Base 百分比，例如 `86.5`。
4. 輸入起始與結束時間，例如 `10:21`、`10:37`。

它會輸出 Base `%/hr`、等效 Base EXP、等效 Base EXP/hr，並記錄到 `BasePercentExpMeter.history.csv`。

### SAVECHAT 輔助估算

雙擊 [啟動SAVECHAT經驗計算.cmd](./啟動SAVECHAT經驗計算.cmd) 可以計算 `/savechat` 產生的經驗資料。程式會讀取 `C:\Gravity\RagnarokZero\Chat\Chat_經驗*.txt`，列出最近的 SAVEDATA；你先選擇要計算的結束點，再確認起點，換算 Base / Job EXP/hr。若輸入目前 Base 等級，會依內建 Lv1~98 升級需求表換算 Base %/hr。
...
若你有直接從遊戲畫面記下起始/結束 Base 百分比與時間，也可以在程式詢問「遊戲畫面起始 Base %」時輸入，例如 `82.8 -> 86.5`、`10:21 -> 10:37`。程式會另外顯示「遊戲%校正」的 `%/hr`，可用來對照 savechat 檔案推算結果。

`/savechat` 只會計算存檔裡還存在的經驗訊息；如果練功時間太久、經驗訊息太多，聊天視窗前段可能已經被洗掉，程式算出的 EXP/hr 會偏低。因此 savechat 版本只能當短時間輔助估算，不適合作為完整經驗紀錄。
...
計算結果會追加到 `SaveChatExpMeter.history.csv`，方便之後回看各段效率。RO 樂園官網在 2026/06/30 公告已開放等級上限 BaseLv110/70；目前內建 Base 經驗需求表依使用者提供的 BaseLv1~99 需求經驗表截圖整理，支援輸入 Lv1~98 並換算到下一級。Lv99 以上尚未找到完整表，因此程式會明確提示，並可讓你手動輸入該等級升下一級的需求經驗來換算 %/hr，避免沿用舊表造成誤差。原始交接筆記已整理到 [docs/RO_EXP_METER_交接筆記.md](./docs/RO_EXP_METER_交接筆記.md)。
